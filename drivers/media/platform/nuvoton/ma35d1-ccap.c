...
/*
 * Nuvoton MA35D1 Camera Capture Interface Controller (CCAP) driver
 *
...
#define   CTL_PKTEN (1<<6)
#define   CTL_PLNEN (1<<5)
...
#define DCULTRA_BASE 0x40260000
enum ccap_status {
	CCAP_IDLE,
	CCAP_INITIALISING,
	CCAP_RUNNING,
			...
			struct ccap_framesize *sd_framesize)
{
	...
	ret =
	    v4l2_subdev_call_state_try(ccap->entity.source, pad, set_fmt,
				       &format);
	if (ret < 0)
		return ret;
				...
				struct v4l2_format *fmt)
{
	...
	case V4L2_PIX_FMT_YUV420M:
		...
		value = ccap_reg_read(ccap_dev, CCAP_PAR);
		value = (value & ~(1 << 7)) | outfmt;
		ccap_reg_write(ccap_dev, CCAP_PAR, value);

		/* Set_Cropping start position for sensor */
		value = ccap_reg_read(ccap_dev, CCAP_CWSP);
		value &= ~(CWSP_CWSADDRV | CWSP_CWSADDRH);
		value |= (0 | 2 << 16);
		ccap_reg_write(ccap_dev, CCAP_CWSP, value);
		...
		value = ccap_reg_read(ccap_dev, CCAP_PLNSL);
		value &= ~(PLNSL_PLNSVNL | PLNSL_PLNSVML);
		value |= (((pix->height / u32GCD0) & 0xff) << 24);
		value |=
		...
		value = ccap_reg_read(ccap_dev, CCAP_PLNSM);
		value &= ~(PLNSM_PLNSVNH | PLNSM_PLNSVMH);
		value |= (((pix->height / u32GCD0) >> 8) << 24);
		value |=
		...
		if (u32GCD0 <= 0)
			u32GCD0 = 1;
		value = ccap_reg_read(ccap_dev, CCAP_PLNSL);
		value &= ~(PLNSL_PLNSHNL | PLNSL_PLNSHML);
		value |= (((pix->width / u32GCD0) & 0xff) << 8);
		value |= ((ccap_dev->fmt.fmt.pix.width / u32GCD0) & 0xff);
		ccap_reg_write(ccap_dev, CCAP_PLNSL, value);
		...
		if (u32GCD0 <= 0)
			u32GCD0 = 1;
		value = ccap_reg_read(ccap_dev, CCAP_PLNSM);
		value &= ~(PLNSM_PLNSHNH | PLNSM_PLNSHMH);
		value |= (((pix->width / u32GCD0) >> 8) << 8);
		value |= ((ccap_dev->fmt.fmt.pix.width / u32GCD0) >> 8);
		ccap_reg_write(ccap_dev, CCAP_PLNSM, value);
		/* Frame Output Pixel Stride Width Register(Planar) */
		value = ccap_reg_read(ccap_dev, CCAP_STRIDE);
		value &= ~STRIDE_PLNSTRIDE;
		value |= ((pix->bytesperline / 2) << 16);
		ccap_reg_write(ccap_dev, CCAP_STRIDE, value);
...
/* Set CCAP Polarity */
	value = ccap_reg_read(ccap_dev, CCAP_PAR);
	value &= ~(VSP_HI | HSP_HI | PCLKP_HI);
	value |= ccap_dev->pdata->polarity;
	ccap_reg_write(ccap_dev, CCAP_PAR, value);
...
static irqreturn_t ccap_isr(int irq, void *dev_id)
{
	...
	if (ccap_dev->status == CCAP_OVERLAY) {
		...
		if (ccap_dev->dcultra_framebase ==
			...
			__raw_writel(ccap_dev->dcultra_framebase,
				     ccap_dev->dcultra_base + 0x1400);
			ccap_reg_write(ccap_dev,
				       CCAP_PKTBA0,
	...
	ccap_dev->active =
	    list_entry(ccap_dev->buf_list.next, struct ccap_buffer, list);
...
static int ccap_set_fmt(struct ccap_device *ccap, struct v4l2_format *f)
{
	...
	ret = ccap_pipeline_s_fmt(ccap, &format);
	if (ret < 0)
		return ret;
	dev_dbg(ccap->dev,
				...
				struct v4l2_frmsizeenum *fsize)
{
	...
	ret =
	    v4l2_subdev_call(ccap_dev->entity.source, pad,
...
static int ccap_overlay(struct file *file, void *priv, unsigned int on)
{
	...
	if (on) {
		...
		ccap_dev->dcultra_framebase =
		    __raw_readl(ccap_dev->dcultra_base + 0x1400);
		ccap_reg_write(ccap_dev,
			       CCAP_PKTBA0, ccap_dev->dcultra_framebase);
		/* Get dcultra frame size */
		framesize = __raw_readl(ccap_dev->dcultra_base + 0x1810);
		ccap_dev->dcultra_framesize =
		    ((framesize >> 15) & 0xffff) *
		    (framesize & 0xffff) * ccap_fmt[ccap_dev->pix_idx].bpl;
		/* Enable CAP engine */
		value = ccap_reg_read(ccap_dev, CCAP_CTL);
		value |= (engine << 0);
		value |= (ccap_dev->pdata->packet << 6);
		value |= (ccap_dev->pdata->planar << 5);
		ccap_reg_write(ccap_dev, CCAP_CTL, value);
		ccap_dev->status = CCAP_OVERLAY;
	} else {
		unsigned int value;
		/* Disable output */
		if ((ccap_reg_read(ccap_dev, CCAP_CTL)
		     && (CTL_PKTEN | CTL_PLNEN)) != 0) {
			value = ccap_reg_read(ccap_dev, CCAP_CTL) | (1 << 16);
			ccap_reg_write(ccap_dev, CCAP_CTL, value);
			/* waiting for the current frame complete */
			while (ccap_reg_read(ccap_dev, CCAP_CTL) & 0x1)
			     ...
			     struct v4l2_async_subdev *asd)
{
	...
	ret =
	    media_create_pad_link(&subdev->entity,
				  src_pad,
				  &ccap->vdev->entity, 0,
...
static int ccap_get_sensor_bounds(struct
				  ccap_device
				  *ccap, struct v4l2_rect
				  *r)
{
	 ...
	 */
	ret =
	    v4l2_subdev_call(ccap->entity.source, pad,
			     get_selection, NULL, &bounds);
	if (!ret)
		*r = bounds.r;
	if (ret != -ENOIOCTLCMD)
		return ret;
...
static int ccap_framesizes_init(struct ccap_device *ccap)
{
	...
	ccap->sd_framesizes =
	    devm_kcalloc(ccap->dev, num_fsize,
			 sizeof(struct ccap_framesize), GFP_KERNEL);
	...
	for (i = 0; i < ccap->num_of_sd_framesizes; i++) {
		fse.index = i;
		ret =
		    v4l2_subdev_call(subdev, pad, enum_frame_size, NULL, &fse);
		if (ret)
			return ret;
...
static const struct
v4l2_async_notifier_operations ccap_sensor_ops = {
...
static const struct
v4l2_file_operations ccap_fops = {
	.owner = THIS_MODULE,
	.open = ccap_open,
			     ...
			     struct ccap_device *ccap_dev)
{
	...
	if (ep.bus_type & V4L2_MBUS_PARALLEL)
		...
		VDEBUG("unknown mode\n");
	VDEBUG("hsync-active=%d\n", (bus->flags & V4L2_MBUS_HSYNC_ACTIVE_HIGH)
	       ? 1 : 0);
	VDEBUG("vsync-active=%d\n", (bus->flags & V4L2_MBUS_VSYNC_ACTIVE_HIGH)
	       ? 1 : 0);
	VDEBUG("pclk-sample=%d\n", (bus->flags & V4L2_MBUS_PCLK_SAMPLE_RISING)
	       ? 1 : 0);
	VDEBUG("bus-width=%d\n", bus->bus_width);
	return 0;
}
...
static int ccap_graph_init(struct ccap_device *ccap)
{
	...
	v4l2_async_nf_init(&ccap->notifier);
	...
	ret = v4l2_async_nf_register(&ccap->v4l2_dev, &ccap->notifier);
	if (ret < 0) {
...
static int ccap_probe(struct platform_device *pdev)
{
	...
	snprintf(ccap_dev->mdev.bus_info,
		 sizeof(ccap_dev->mdev.bus_info), "platform:%s", DRV_NAME);
	ccap_dev->mdev.dev = &pdev->dev;
	media_device_init(&ccap_dev->mdev);
	ccap_dev->dev = &pdev->dev;
	ccap_dev->status = CCAP_INITIALISING;
	ccap_dev->pix_idx = 1;
	ccap_dev->dcultra_base = ioremap(DCULTRA_BASE, 0x2000);
	pix = &ccap_dev->pix;
	/* Fill in defaults */
	...
	ret =
	    media_entity_pads_init(&ccap_dev->vdev->entity,
...
static int ccap_remove(struct platform_device
		       *pdev)
{
	struct v4l2_device *v4l2_dev = platform_get_drvdata(pdev);
	struct ccap_device *ccap_dev = container_of(v4l2_dev, struct
						    ccap_device, v4l2_dev);
	ENTRY();
	pm_runtime_disable(&pdev->dev);
